...
=begin rdoc
	...
	para tratar de conseguir una solución mejor a través de la heuristica
=end
	class MMDP < BasicMMDP
		# El punto de ruptura representa el valor máximo de búsqueda
		# dentro de la búsqueda local. Si este valor es igual a 2
		# significara que se buscara localmente a través de la mitad
		# del vector solución dado por la búsqueda global. Si es
		# tres, sera a través de la tercera parte, y así sucesivamente
		attr_reader :punto_ruptura

		# Inicializa la clase para que cargue los valores de la base de datos
		# Recibe como parámetro un string indicando el lugar de donde
		...
		# Realiza una búsqueda global para tratar de obtener un
		# vector con la máxima distancia entre nodos posible
		def busqueda_global
			return generar_solucion_aleatoria
		end

		# Genera una solución de forma aleatoria y trata de mejorarla mediante
		# las técnicas de búsqueda local. 
		#
		# Puede recibir como parámetro el tipo
		# de búsqueda local a recibir que puede ser :best_improvement para
		...
		# primera mejora de los vecinos. También puede usar como tipo
		# el valor :enfriamiento_simulado para activar el algoritmo de
		# enfriamiento simulado
		#
		# Si no se especifica el tipo de búsqueda se asume que se prefiere
		# la técnica Best improvement
		def generar_solucion_busqueda_local(tipo = :first_improvement)
			raise TypeError, "Tipo debe de ser un símbolo" unless tipo.kind_of? Symbol
